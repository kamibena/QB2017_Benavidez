...
portal=portal[!(portal$taxa %in% c("Bird","Rabbit","Reptile")),]
...
tbs1981.db=vegdist(tbs.1981,method="bray")
...
rodentREL=tbs.1981
  ...
  for(i in 1:nrow(tbs.1981)){
    rodentREL[i, ] = tbs.1981[i, ] / sum(tbs.1981[i, ]) 
  }
...
> ***Answer 4d***: The autoregressive model worked the best.  This would say that 
...
> Studying biodiversity through space and time can be challenging, especially if you want to study make future forcasts.  This is primarily true because it is very difficult to account for all ever-changing variables that might be influencing biodiversity in any given area.  For exmaple, if you are attempting to forcast how diversity and age class of trees in a park will look in the future, how do you account managment practices of the city in which the park is located?  How can you predict if funding will even allow for management?  Furthermore, identifying the appropriate scope of a project is difficult as well - how much/little space and time is necessary to adequatly measure the phenomonon you are attempting to describe?   
